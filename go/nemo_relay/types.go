...
// functions that correspond to the C FFI types exposed by nemo-relay-ffi. Each
...
extern char* nemo_relay_scope_handle_uuid(const FfiScopeHandle* ptr);
extern char* nemo_relay_scope_handle_name(const FfiScopeHandle* ptr);
extern int32_t nemo_relay_scope_handle_scope_type(const FfiScopeHandle* ptr);
extern uint32_t nemo_relay_scope_handle_attributes(const FfiScopeHandle* ptr);
extern char* nemo_relay_scope_handle_parent_uuid(const FfiScopeHandle* ptr);
extern char* nemo_relay_scope_handle_data(const FfiScopeHandle* ptr);
extern char* nemo_relay_scope_handle_metadata(const FfiScopeHandle* ptr);
extern void nemo_relay_scope_handle_free(FfiScopeHandle* ptr);

// Accessors — ToolHandle
extern char* nemo_relay_tool_handle_uuid(const FfiToolHandle* ptr);
extern char* nemo_relay_tool_handle_name(const FfiToolHandle* ptr);
extern uint32_t nemo_relay_tool_handle_attributes(const FfiToolHandle* ptr);
extern char* nemo_relay_tool_handle_parent_uuid(const FfiToolHandle* ptr);
extern void nemo_relay_tool_handle_free(FfiToolHandle* ptr);

// Accessors — LLMHandle
extern char* nemo_relay_llm_handle_uuid(const FfiLLMHandle* ptr);
extern char* nemo_relay_llm_handle_name(const FfiLLMHandle* ptr);
extern uint32_t nemo_relay_llm_handle_attributes(const FfiLLMHandle* ptr);
extern char* nemo_relay_llm_handle_parent_uuid(const FfiLLMHandle* ptr);
extern void nemo_relay_llm_handle_free(FfiLLMHandle* ptr);
...
extern void nemo_relay_llm_request_free(FfiLLMRequest* ptr);

// Event accessors
extern char* nemo_relay_event_uuid(const FfiEvent* ptr);
extern char* nemo_relay_event_name(const FfiEvent* ptr);
extern char* nemo_relay_event_kind(const FfiEvent* ptr);
extern char* nemo_relay_event_json(const FfiEvent* ptr);
extern char* nemo_relay_event_atof_version(const FfiEvent* ptr);
extern char* nemo_relay_event_scope_category(const FfiEvent* ptr);
extern char* nemo_relay_event_category(const FfiEvent* ptr);
extern uint32_t nemo_relay_event_attributes(const FfiEvent* ptr);
extern char* nemo_relay_event_attributes_json(const FfiEvent* ptr);
extern char* nemo_relay_event_category_profile(const FfiEvent* ptr);
extern char* nemo_relay_event_data(const FfiEvent* ptr);
extern char* nemo_relay_event_data_schema(const FfiEvent* ptr);
extern char* nemo_relay_event_metadata(const FfiEvent* ptr);
extern char* nemo_relay_event_timestamp(const FfiEvent* ptr);
extern char* nemo_relay_event_input(const void* ptr);
extern char* nemo_relay_event_output(const void* ptr);
extern char* nemo_relay_event_model_name(const void* ptr);
extern char* nemo_relay_event_tool_call_id(const void* ptr);
extern char* nemo_relay_event_parent_uuid(const void* ptr);
extern char* nemo_relay_event_scope_type(const void* ptr);
extern char* nemo_relay_event_annotated_request(const FfiEvent* ptr);
extern char* nemo_relay_event_annotated_response(const FfiEvent* ptr);
extern void nemo_relay_event_free(FfiEvent* ptr);
...
func newLLMRequestFromPtr(ptr unsafe.Pointer) *LLMRequest {
	...
	runtime.SetFinalizer(r, func(r *LLMRequest) {
		if r.ptr != nil {
			C.nemo_relay_llm_request_free(r.ptr)
			r.ptr = nil
		}
	})
	return r
...
var newLLMRequestFunc = func(headersJSON, contentJSON string) *LLMRequest {
	...
	return newLLMRequestFromPtr(unsafe.Pointer(C.nemo_relay_llm_request_new(cHeaders, cContent)))
}
...
func newScopeHandle(ptr *C.FfiScopeHandle) *ScopeHandle {
	...
	runtime.SetFinalizer(h, func(h *ScopeHandle) {
		if h.ptr != nil {
			C.nemo_relay_scope_handle_free(h.ptr)
			h.ptr = nil
		}
	})
	return h
...
func newToolHandle(ptr *C.FfiToolHandle) *ToolHandle {
	...
	runtime.SetFinalizer(h, func(h *ToolHandle) {
		if h.ptr != nil {
			C.nemo_relay_tool_handle_free(h.ptr)
			h.ptr = nil
		}
	})
	return h
...
func (h *ToolHandle) UUID() string { return goString(C.nemo_relay_tool_handle_uuid(h.ptr)) }

// Name returns the name of the tool being called.
func (h *ToolHandle) Name() string { return goString(C.nemo_relay_tool_handle_name(h.ptr)) }

// Attributes returns the attribute bitflags for this tool call.
func (h *ToolHandle) Attributes() uint32 {
	return uint32(C.nemo_relay_tool_handle_attributes(h.ptr))
}

// ParentUUID returns the UUID of the parent scope for this tool call.
func (h *ToolHandle) ParentUUID() string {
	return goStringOpt(C.nemo_relay_tool_handle_parent_uuid(h.ptr))
}
...
func newLLMHandle(ptr *C.FfiLLMHandle) *LLMHandle {
	...
	runtime.SetFinalizer(h, func(h *LLMHandle) {
		if h.ptr != nil {
			C.nemo_relay_llm_handle_free(h.ptr)
			h.ptr = nil
		}
	})
	return h
...
func (h *LLMHandle) UUID() string { return goString(C.nemo_relay_llm_handle_uuid(h.ptr)) }

// Name returns the name of the LLM being called.
func (h *LLMHandle) Name() string { return goString(C.nemo_relay_llm_handle_name(h.ptr)) }

// Attributes returns the attribute bitflags for this LLM call.
func (h *LLMHandle) Attributes() uint32 {
	return uint32(C.nemo_relay_llm_handle_attributes(h.ptr))
}

// ParentUUID returns the UUID of the parent scope for this LLM call.
func (h *LLMHandle) ParentUUID() string {
	return goStringOpt(C.nemo_relay_llm_handle_parent_uuid(h.ptr))
}
...
func (r *LLMRequest) Headers() json.RawMessage {
	return goJSONOpt(C.nemo_relay_llm_request_headers(r.ptr))
}

// Content returns the request content as raw JSON.
func (r *LLMRequest) Content() json.RawMessage {
	return goJSONOpt(C.nemo_relay_llm_request_content(r.ptr))
}
...
func (e eventBase) UUID() string {
	...
	return goString(C.nemo_relay_event_uuid(e.ptr))
}
...
func (e eventBase) Name() string {
	...
	return goStringOpt(C.nemo_relay_event_name(e.ptr))
}
...
func (e eventBase) Kind() string {
	...
	return goStringOpt(C.nemo_relay_event_kind(e.ptr))
}
func (e eventBase) ATOFVersion() string {
	if e.snapshot != nil {
		return e.snapshot.atofVersion
	}
	return goStringOpt(C.nemo_relay_event_atof_version(e.ptr))
}
func (e eventBase) ScopeCategory() string {
	if e.snapshot != nil {
		return e.snapshot.scopeCategory
	}
	return goStringOpt(C.nemo_relay_event_scope_category(e.ptr))
}
func (e eventBase) Category() string {
	if e.snapshot != nil {
		return e.snapshot.category
	}
	return goStringOpt(C.nemo_relay_event_category(e.ptr))
}
...
func newEvent(ptr *C.FfiEvent) Event {
	base := eventBase{
		snapshot: &eventSnapshot{
			kind:              goStringOpt(C.nemo_relay_event_kind(ptr)),
			atofVersion:       goStringOpt(C.nemo_relay_event_atof_version(ptr)),
			scopeCategory:     goStringOpt(C.nemo_relay_event_scope_category(ptr)),
			category:          goStringOpt(C.nemo_relay_event_category(ptr)),
			uuid:              goString(C.nemo_relay_event_uuid(ptr)),
			name:              goStringOpt(C.nemo_relay_event_name(ptr)),
			parentUUID:        goStringOpt((*C.char)(C.nemo_relay_event_parent_uuid(unsafe.Pointer(ptr)))),
			scopeType:         goStringOpt((*C.char)(C.nemo_relay_event_scope_type(unsafe.Pointer(ptr)))),
			attributes:        uint32(C.nemo_relay_event_attributes(ptr)),
			attributesJSON:    goJSONOpt(C.nemo_relay_event_attributes_json(ptr)),
			categoryProfile:   goJSONOpt(C.nemo_relay_event_category_profile(ptr)),
			data:              goJSONOpt(C.nemo_relay_event_data(ptr)),
			dataSchema:        goJSONOpt(C.nemo_relay_event_data_schema(ptr)),
			metadata:          goJSONOpt(C.nemo_relay_event_metadata(ptr)),
			timestamp:         goString(C.nemo_relay_event_timestamp(ptr)),
			input:             goJSONOpt((*C.char)(C.nemo_relay_event_input(unsafe.Pointer(ptr)))),
			output:            goJSONOpt((*C.char)(C.nemo_relay_event_output(unsafe.Pointer(ptr)))),
			modelName:         goStringOpt((*C.char)(C.nemo_relay_event_model_name(unsafe.Pointer(ptr)))),
			toolCallID:        goStringOpt((*C.char)(C.nemo_relay_event_tool_call_id(unsafe.Pointer(ptr)))),
			annotatedRequest:  goJSONOpt(C.nemo_relay_event_annotated_request(ptr)),
			annotatedResponse: goJSONOpt(C.nemo_relay_event_annotated_response(ptr)),
			eventJSON:         goJSONOpt(C.nemo_relay_event_json(ptr)),
		},
	}
	switch base.Kind() {
	case "scope":
