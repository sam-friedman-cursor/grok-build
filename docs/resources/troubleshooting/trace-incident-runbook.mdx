...
description: "Diagnose missing, partial, duplicate, or mis-scoped NeMo Relay traces."
position: 2
---
...
Use [Observability](/configure-plugins/observability/about),
[Observability Configuration](/configure-plugins/observability/configuration), and
...
For typed OpenTelemetry export, confirm these settings:

- Each endpoint has the intended `full`, `gen_ai`, or `openinference` type.
- The OpenTelemetry Protocol (OTLP) endpoint, headers, `header_env` variables,
  credentials, and network egress are available in the target environment.
...
Refer to [Agent Trajectory Observability Format (ATOF)](/configure-plugins/observability/atof),
