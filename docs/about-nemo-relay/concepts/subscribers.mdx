...
`runtimeDiagnostics()`. The C FFI exposes the corresponding
`*_runtime_diagnostics_json` APIs, which return a caller-owned JSON array
released with `nemo_relay_string_free`. These snapshots report stable codes,
occurrence counts, and the most recent message for delivery and
event-processing failures. See
