...
- Managed tool surfaces are sanitized as JSON payloads with exact JSON-pointer
  targeting or restricted single-segment glob selectors.
...
- The plugin supports exact JSON Pointer targeting plus a restricted,
  single-segment JSON Pointer glob selector. It does not support recursive
  selectors, predicates, or arbitrary JSONPath expressions.
